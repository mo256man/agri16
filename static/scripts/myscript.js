...
async function getContec(isTry, isLightCnt) {
    let msg = "";
    let isLightChange = false;                                          // 育成LEDオンオフ状態変化するかどうか　初期値はfalse
    ...
    if (isLightChange) {                        // LED点灯消灯状態変化していたら
        await enpowerLED(isLED);                // LEDを制御する
    };
...
// 育成LEDを光らせる
async function enpowerLED(bool) {
